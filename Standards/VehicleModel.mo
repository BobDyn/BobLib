...
model VehicleModel
  ...
  // Toggle controllers
  parameter Boolean closedLoopRadius = false "Whether to control turn radius to a target profile" annotation(Evaluate = false);
  parameter Boolean closedLoopVelocity = false "Whether to control velocity to a target profile" annotation(Evaluate = false);
  
  // Number of points in each profile
  parameter Integer numPoints = 1000;
  
  // Closed-loop parameters
  parameter SIunits.Length curvTimeTable[numPoints, 2] = {if j == 1 then i / numPoints else 0.0 for j in 1:2, i in 1:numPoints} "Target maneuver curvature profile [t1, curv1; t2, curv2; ...]" annotation(Evaluate = false);
  parameter SIunits.Velocity velTimeTable[numPoints, 2] = {if j == 1 then i / numPoints else 10.0 for j in 1:2, i in 1:numPoints} "Target maneuver velocity profile [t1, v1; t2, v2; ...]" annotation(Evaluate = false);
  
  parameter Real curvGain = 3 "Proportional gain of curvature controller" annotation(Evaluate = false);
  parameter Real curvTi = 0.02 "Time constant of curvature controller" annotation(Evaluate = false);
  
  parameter Real velGain = 200 "Proportional gain of velocity controller" annotation(Evaluate = false);
  parameter Real velTi = 1 "Time constant of velocity controller" annotation(Evaluate = false);

  // Open-loop parameters
  parameter SIunits.Angle steerTimeTable[numPoints, 2] = {if j == 1 then i / numPoints else 0.0 for j in 1:2, i in 1:numPoints} "Input handwheel angle profile [t1, hwa1; t2, hwa2; ...]" annotation(Evaluate = false);
  parameter SIunits.Torque driveTorqueTimeTable[numPoints, 2] = {if j == 1 then i / numPoints else 0.0 for j in 1:2, i in 1:numPoints} "Input drive torque profile [t1, tau1; t2, tau2; ...]" annotation(Evaluate = false);
  ...
  Real curvErrorInt(start = 0, fixed = true);
  Real velErrorInt(start = 0, fixed = true);
  
  Real frSteerApplied;
  Real driveTorqueApplied;
  ...
  // Steer and drive torque sources
  Modelica.Blocks.Sources.RealExpression frSteerExpression(y = frSteerApplied) annotation(
    Placement(transformation(origin = {-70, 110}, extent = {{-10, -10}, {10, 10}})));
  Modelica.Blocks.Sources.RealExpression driveTorqueExpression(y = driveTorqueApplied)  annotation(
...
equation
  curvature = vehicle.chassis.spaceFrame.sprungBody.w_a[3]/max(velocity, 0.1);
  velocity = norm({bodyVels[1], bodyVels[2], 0});  // I recognize this is approximate and out of plane by the frame attitude. PR a fix ;)
  
  if closedLoopRadius then
    curvError = smooth(1, min(1, max(0, (time - 1)/0.2)))*(interpolate(curvTimeTable[:, 1], curvTimeTable[:, 2], time) - curvature);
    der(curvErrorInt) = curvError / curvTi;
    frSteerApplied = curvGain * (curvErrorInt + curvError);
  else
    curvError = 0;
    der(curvErrorInt) = 0;
    frSteerApplied = interpolate(steerTimeTable[:, 1], steerTimeTable[:, 2], time);
  end if;
  
  if closedLoopVelocity then
    velError = interpolate(velTimeTable[:, 1], velTimeTable[:, 2], time) - velocity;
    der(velErrorInt) = velError / velTi;
    driveTorqueApplied = velGain * (velErrorInt + velError);
  else
    velError = 0;
    der(velErrorInt) = 0;
    driveTorqueApplied = interpolate(driveTorqueTimeTable[:, 1], driveTorqueTimeTable[:, 2], time);
  end if;
  ...
  connect(frSteerExpression.y, frSteerPosition.phi_ref) annotation(
    Line(points = {{-58, 110}, {-42, 110}}, color = {0, 0, 127}));
  connect(driveTorqueExpression.y, vehicle.uPTNTorque) annotation(
    Line(points = {{-8, -50}, {0, -50}, {0, -34}}, color = {0, 0, 127}));
  annotation(
