...
model VehicleModel2
  ...
  parameter DWDirect_DWDirectRecord fr_pVehicle;
  ...
  parameter Modelica.SIunits.Time steerStart = 1.0 "Start time" annotation(
    Evaluate = false);
  
  // Closed-loop parameters
  parameter SIunits.Length targetRad = 20 "Target maneuver curvature" annotation(
    Evaluate = false,
    Dialog(enable = closedLoopRadius));
  parameter SIunits.Velocity targetVel = 15 "Target maneuver velocity" annotation(
    Evaluate = false,
    Dialog(enable = closedLoopVelocity));
  parameter SIunits.Velocity initialVel = targetVel "Initial velocity" annotation(
    Evaluate = false);
  parameter Real curvGain = 3 "Proportional gain of curvature controller" annotation(
    Evaluate = false,
    Dialog(enable = closedLoopRadius));
  parameter Real curvTi = 0.02 "Time constant of curvature controller" annotation(
    Evaluate = false,
    Dialog(enable = closedLoopRadius));
  parameter Real velGain = 200 "Proportional gain of velocity controller" annotation(
    Evaluate = false,
    Dialog(enable = closedLoopVelocity));
  parameter Real velTi = 1 "Time constant of velocity controller" annotation(
    Evaluate = false,
    Dialog(enable = closedLoopVelocity));
  parameter Real radErrorTol = 0.002 "ISO4138 radius error tolerance" annotation(
    Evaluate = false,
    Dialog(enable = closedLoopRadius));
  ...
  parameter SIunits.Angle steerAmp = 6*Modelica.Constants.pi/180 "Amplitude" annotation(
    Evaluate = false);
  parameter SIunits.Frequency steerFreq = 1.0 "Frequency (Hz)" annotation(
    Evaluate = false);
  ...
  // Vehicle
  BobLib.Vehicle.Vehicle_DWDirect_DWBCStabar vehicle(fr_pVehicle = fr_pVehicle, rr_pVehicle = rr_pVehicle) annotation(
    Placement(transformation(origin = {0, 20}, extent = {{-45, -50}, {45, 50}})));
  
//  BobLib.Vehicle.Vehicle_DWDirect_DWDirect vehicle(fr_pVehicle = pVehicle) annotation(
//    Placement(transformation(origin = {0, 20}, extent = {{-45, -50}, {45, 50}})));
  
  
  // CG motion
  Modelica.Mechanics.MultiBody.Joints.FreeMotion cgFreeMotion(animation = false, r_rel_a(start = {0, 0, 0}, each fixed = true), enforceStates = false, v_rel_a(start = {initialVel, 0, 0}, each fixed = true)) annotation(
  ...
  final parameter Real cpInitFL[3] = fr_pVehicle.pFrDW.wheelCenter + Frames.resolve1(Frames.axesRotations({1, 2, 3}, {fr_pVehicle.pFrPartialWheel.staticGamma*pi/180, 0, fr_pVehicle.pFrPartialWheel.staticAlpha*pi/180}, {0, 0, 0}), {0, 0, -fr_pVehicle.pFrPartialWheel.R0});
  final parameter Real cpInitFR[3] = Vector.mirrorXZ(cpInitFL);
  final parameter Real cpInitRL[3] = fr_pVehicle.pRrDW.wheelCenter + Frames.resolve1(Frames.axesRotations({1, 2, 3}, {fr_pVehicle.pRrPartialWheel.staticGamma*pi/180, 0, fr_pVehicle.pRrPartialWheel.staticAlpha*pi/180}, {0, 0, 0}), {0, 0, -fr_pVehicle.pRrPartialWheel.R0});
  final parameter Real cpInitRR[3] = Vector.mirrorXZ(cpInitRL);
  //  Modelica.Blocks.Sources.CombiTimeTable frSteerTimeTable(table = [0, 0; 1, 0], columns = {2}, smoothness = Modelica.Blocks.Types.Smoothness.LinearSegments, extrapolation = Modelica.Blocks.Types.Extrapolation.HoldLastPoint)  annotation(
...
protected
  ...
  Modelica.Mechanics.MultiBody.Parts.Fixed fixedRR(r = cpInitRR, animation = false) annotation(
    Placement(transformation(origin = {130, -50}, extent = {{10, -10}, {-10, 10}})));
  Modelica.Mechanics.MultiBody.Parts.Fixed cgFixed(r = fr_pVehicle.pSprungMass.rCM, animation = false) annotation(
  ...
  Modelica.Blocks.Continuous.PI curvPI(k = curvGain, T = curvTi, initType = Modelica.Blocks.Types.Init.InitialOutput) annotation(
  ...
  Modelica.Blocks.Continuous.PI speedPI(k = velGain, T = velTi) annotation(
    Placement(transformation(origin = {-30, -50}, extent = {{-10, -10}, {10, 10}})));
initial equation
  vehicle.chassis.frAxleDW.leftTire.wheelModel.hubAxis.w = initialVel/fr_pVehicle.pFrPartialWheel.R0;
  vehicle.chassis.frAxleDW.rightTire.wheelModel.hubAxis.w = initialVel/fr_pVehicle.pFrPartialWheel.R0;
  vehicle.chassis.rrAxleDW.leftTire.wheelModel.hubAxis.w = initialVel/fr_pVehicle.pRrPartialWheel.R0;
  vehicle.chassis.rrAxleDW.rightTire.wheelModel.hubAxis.w = initialVel/fr_pVehicle.pRrPartialWheel.R0;
...
// Speed quantities
  speed = norm(bodyVels);
// I recognize this is approximate and out of plane by the frame attitude. PR a fix ;)
