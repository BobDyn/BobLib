...
partial model ChassisBase
  ...
  parameter SIunits.Position pVehicleCG[3];
  ...
  // Rear axle
  replaceable BobLib.Vehicle.Chassis.Suspension.RrAxleDW_BC_Stabar rrAxleDW annotation(
  ...
  // Sprung mass
  Modelica.Mechanics.MultiBody.Parts.Body sprungBody(m = pSprungMass.m,
                                                     r_CM = {0, 0, 0},
                                                     I_11 = pSprungMass.inertia[1, 1],
                                                     I_22 = pSprungMass.inertia[2, 2],
                                                     I_33 = pSprungMass.inertia[3, 3],
                                                     I_21 = pSprungMass.inertia[2, 1],
                                                     I_31 = pSprungMass.inertia[3, 1],
                                                     I_32 = pSprungMass.inertia[3, 2],
...
protected
  Modelica.Mechanics.MultiBody.Parts.FixedTranslation toVehicleCG(r = pVehicleCG - frAxleDW.effectiveCenter, animation = false) annotation(
    Placement(transformation(origin = {50, 0}, extent = {{-10, -10}, {10, 10}}, rotation = -0)));
  Modelica.Mechanics.MultiBody.Parts.FixedTranslation toSprungBody(r = pSprungMass.rCM - frAxleDW.effectiveCenter, animation = false) annotation(
...
equation
  ...
  connect(toVehicleCG.frame_b, cgFrame) annotation(
