...
model Bellcrank3
  ...
  parameter SIunits.Position rShock[3] "Coordinates of shock pickup, resolved in world frame" annotation(
    Dialog(group = "Geometry"));
  parameter SIunits.Position rStabar[3] "Coordinates of stabar pickup, resolved in world frame" annotation(
    Dialog(group = "Geometry"));
  
  parameter Integer rodPickup(min=1, max=3) = 1 "Push/pullrod pickup mapping, where 1 is the most counter-clockwise pickup about the left bellcrank (generally with the lowest Z coordinate)" annotation(Dialog(group="Pickup Mapping"));
  parameter Integer shockPickup(min=1, max=3) = 2 "Shock pickup mapping, where 1 is the most counter-clockwise pickup about the left bellcrank (generally with the lowest Z coordinate)" annotation(Dialog(group="Pickup Mapping"));
  parameter Integer stabarPickup(min=1, max=3) = 3 "Stabar pickup mapping, where 1 is the most counter-clockwise pickup about the left bellcrank (generally with the lowest Z coordinate)" annotation(Dialog(group="Pickup Mapping"));
  ...
  final parameter SIunits.Position pickup_1[3] = if (stabarPickup == 1) then rStabar elseif (rodPickup == 1) then rRod elseif (shockPickup == 1) then rShock else {0, 0, 0};
  final parameter SIunits.Position pickup_2[3] = if (stabarPickup == 2) then rStabar elseif (rodPickup == 2) then rRod elseif (shockPickup == 2) then rShock else {0, 0, 0};
  final parameter SIunits.Position pickup_3[3] = if (stabarPickup == 3) then rStabar elseif (rodPickup == 3) then rRod elseif (shockPickup == 3) then rShock else {0, 0, 0};
...
protected
  ...
  Modelica.Mechanics.MultiBody.Parts.FixedTranslation toThirdPickup(final r = pickup_3 - pivot, final extra = 0.0, animation = false) annotation(
    Placement(transformation(origin = {-40, 50}, extent = {{-10, -10}, {10, 10}})));

initial equation
  assert(
    stabarPickup <> rodPickup and
    stabarPickup <> shockPickup and
...
equation
  ...
  connect(toThirdPickup.frame_b, pickupFrame3) annotation(
    Line(points = {{-30, 50}, {0, 50}, {0, 100}}, color = {95, 95, 95}));
  ...
  connect(side_4.frame_a, toThirdPickup.frame_b) annotation(
    Line(points = {{-20, 70}, {-10, 70}, {-10, 50}, {-30, 50}}, color = {95, 95, 95}));
  connect(toThirdPickup.frame_a, revolute.frame_b) annotation(
    Line(points = {{-50, 50}, {-56, 50}, {-56, 0}, {-60, 0}}, color = {95, 95, 95}));
