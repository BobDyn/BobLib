...
model TestRrAxleDW
  ...
  // Steer input
  Modelica.Blocks.Sources.Ramp steerRamp(duration = 1, height = 0, startTime = 1) annotation(
  ...
  // Left jounce input
  Modelica.Blocks.Sources.Ramp leftJounceRamp(duration = 1, height = 1*0.0254, startTime = 1) annotation(
    Placement(transformation(origin = {-110, 0}, extent = {{-10, -10}, {10, 10}}, rotation = -0)));
  
  // Right jounce input
  Modelica.Blocks.Sources.Ramp rightJounceRamp(duration = 1, height = -1*0.0254, startTime = 1) annotation(
    Placement(transformation(origin = {110, 0}, extent = {{10, -10}, {-10, 10}}, rotation = -0)));
  
  // Front axle
  BobLib.Vehicle.Chassis.Suspension.RrAxleDW_BC_ARB frAxleDW(pAxle = pVehicle.pRrAxleDW,
