within BobLibVehicleInterfaces.Chassis.Suspension.Templates.SteeringRack;

model RackAndPinion "Rack and pinion"
  extends BobLibVehicleInterfaces.Icons.RackAndPinionIcon;
  ...
  Modelica.Mechanics.MultiBody.Parts.FixedTranslation fromLeft(r = {0, -pRack.leftPickup[2], 0}, width = linkDiameter, height = linkDiameter, animation = not headless)  annotation(
    Placement(transformation(origin = {-30, 0}, extent = {{-10, -10}, {10, 10}}, rotation = -0)));
  Modelica.Mechanics.MultiBody.Parts.FixedTranslation toRight(r = {0, -pRack.leftPickup[2], 0}, width = linkDiameter, height = linkDiameter, animation = not headless)  annotation(
  ...
  // Visualization
  Modelica.Mechanics.MultiBody.Visualizers.FixedShape pinionVisualizer(length = 2*pRack.cFactor/(2*Modelica.Constants.pi), width = 1.25*pRack.cFactor/(2*Modelica.Constants.pi), shapeType = "cylinder", lengthDirection = {0, 0, 1}, height = 1.25*pRack.cFactor/(2*Modelica.Constants.pi), color = {255, 255, 255}, animation = not headless)  annotation(
    Placement(transformation(origin = {-30, -70}, extent = {{10, -10}, {-10, 10}})));
  Modelica.Mechanics.MultiBody.Visualizers.FixedShape travelVisualizer(shapeType = "cylinder", lengthDirection = {0, 1, 0}, widthDirection = {0, 0, 1}, length = linkDiameter, width = linkDiameter*1.1, height = linkDiameter*1.1, color = {0, 0, 0}, r_shape = {0, -linkDiameter/2, 0}, animation = not headless)  annotation(
...
<p>
Model <code>RackAndPinion</code> converts steering shaft rotation into rack and tie-rod motion.
</p>
<p>
Axle models use it to drive left and right steering pickups from the VehicleInterfaces steering input while preserving the multibody rack support path.
</p>
</html>"));
end RackAndPinion;
