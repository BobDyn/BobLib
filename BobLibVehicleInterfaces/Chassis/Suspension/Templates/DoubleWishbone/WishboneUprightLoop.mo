within BobLibVehicleInterfaces.Chassis.Suspension.Templates.DoubleWishbone;

model WishboneUprightLoop "Kinematic loop consisting of upright, lower wishbone, and upper wishbone"
  extends BobLibVehicleInterfaces.Icons.WishboneUprightLoopIcon;
  ...
  Modelica.Mechanics.MultiBody.Joints.Assemblies.JointUSR upperWishboneUpright(n1_a = {1, 0, 0}, n_b = Vectors.normalize(pDW.upperFore_i - pDW.upperAft_i), rRod1_ia = pDW.upper_o - pDW.lower_o, rRod2_ib = pDW.upper_o - (pDW.upperFore_i + pDW.upperAft_i)/2, sphereDiameter = jointDiameter, rod1Diameter = linkDiameter, rod2Diameter = 0, revoluteDiameter = linkDiameter, revoluteLength = Vectors.norm(pDW.upperFore_i - pDW.upperAft_i) + linkDiameter, cylinderLength = jointDiameter*0.125, cylinderDiameter = jointDiameter*0.125, animation = not headless) annotation(
    Placement(transformation(origin = {-2, 18}, extent = {{20, -20}, {-20, 20}}, rotation = -90)));
  // Lower wisbone
  Modelica.Mechanics.MultiBody.Joints.Revolute lowerJoint_i(n = Vectors.normalize(pDW.lowerFore_i - pDW.lowerAft_i), cylinderLength = Vectors.norm(pDW.lowerFore_i - pDW.lowerAft_i) + linkDiameter, cylinderDiameter = linkDiameter, animation = not headless, phi(nominal = 0.05), w(start = 0, nominal = 1)) annotation(
  ...
  // Steering interface
  Modelica.Mechanics.MultiBody.Joints.Revolute steeringAxis(n = Vectors.normalize(pDW.upper_o - pDW.lower_o), cylinderLength = jointDiameter, cylinderDiameter = jointDiameter, animation = not headless, phi(nominal = 0.1), w(start = 0, nominal = 1)) annotation(
  ...
  Modelica.Mechanics.MultiBody.Visualizers.FixedShape upperForeRod(shapeType = "cylinder", lengthDirection = Frames.resolve2(upperFrameToFore.frame_b.R, upperForeLinkDirection), length = upperForeLinkLength, width = linkDiameter, height = linkDiameter, color = {0, 0, 0}, animation = not headless) annotation(
    Placement(transformation(origin = {-40, 90}, extent = {{-10, -10}, {10, 10}})));
  Modelica.Mechanics.MultiBody.Visualizers.FixedShape upperAftRod(shapeType = "cylinder", lengthDirection = Frames.resolve2(upperFrameToAft.frame_b.R, upperAftLinkDirection), length = upperAftLinkLength, width = linkDiameter, height = linkDiameter, color = {0, 0, 0}, animation = not headless) annotation(
  ...
  Modelica.Mechanics.MultiBody.Visualizers.FixedShape lowerForeRod(shapeType = "cylinder", lengthDirection = Frames.resolve2(lowerFrameToFore.frame_b.R, lowerForeLinkDirection), length = lowerForeLinkLength, width = linkDiameter, height = linkDiameter, color = {0, 0, 0}, animation = not headless) annotation(
    Placement(transformation(origin = {-40, -30}, extent = {{-10, -10}, {10, 10}})));
  Modelica.Mechanics.MultiBody.Visualizers.FixedShape lowerAftRod(shapeType = "cylinder", lengthDirection = Frames.resolve2(lowerFrameToAft.frame_b.R, lowerAftLinkDirection), length = lowerAftLinkLength, width = linkDiameter, height = linkDiameter, color = {0, 0, 0}, animation = not headless) annotation(
    Placement(transformation(origin = {-40, -90}, extent = {{-10, -10}, {10, 10}})));
protected
