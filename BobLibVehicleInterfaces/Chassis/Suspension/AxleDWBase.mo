...
// Interface frames
  ...
  replaceable BobLibVehicleInterfaces.Chassis.Suspension.Tires.BaseTire rightTire(pPartialWheel = pRightPartialWheel) annotation(
    Placement(transformation(origin = {160, 50}, extent = {{-10, -10}, {10, 10}})));
  // Double wishbones
  BobLibVehicleInterfaces.Chassis.Suspension.Templates.DoubleWishbone.WishboneUprightLoop leftWishboneUprightLoop(pDW = pLeftDW, final linkDiameter = linkDiameter, final jointDiameter = jointDiameter) annotation(
    Placement(transformation(origin = {-69, 50}, extent = {{29, -29}, {-29, 29}})));
  BobLibVehicleInterfaces.Chassis.Suspension.Templates.DoubleWishbone.WishboneUprightLoop rightWishboneUprightLoop(pDW = pRightDW, final linkDiameter = linkDiameter, final jointDiameter = jointDiameter) annotation(
    Placement(transformation(origin = {69, 50}, extent = {{-29, -29}, {29, 29}})));
  BobLibVehicleInterfaces.Chassis.Suspension.Linkages.Rod leftTieRod(r_a = pRack.leftPickup, r_b = pLeftDW.tie_o, show_universal_axes = false, kinematicConstraint = true, final linkDiameter = linkDiameter, final jointDiameter = jointDiameter) annotation(
    Placement(transformation(origin = {-60, 100}, extent = {{20, -20}, {-20, 20}})));
  BobLibVehicleInterfaces.Chassis.Suspension.Linkages.Rod rightTieRod(r_a = Vector.mirrorXZ(pRack.leftPickup), r_b = pRightDW.tie_o, show_universal_axes = false, kinematicConstraint = true, final linkDiameter = linkDiameter, final jointDiameter = jointDiameter) annotation(
    Placement(transformation(origin = {60, 100}, extent = {{-20, -20}, {20, 20}})));

  BobLibVehicleInterfaces.Chassis.Suspension.Templates.SteeringRack.RackAndPinion rackAndPinion(pRack = pRack, final linkDiameter = linkDiameter) annotation(
