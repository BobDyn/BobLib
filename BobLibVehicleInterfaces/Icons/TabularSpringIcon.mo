within BobLibVehicleInterfaces.Icons;

partial model TabularSpringIcon "Reusable tabular spring icon"

  annotation(
    Icon(graphics = {Line(origin = {27.89, 0}, points = {{-127.885, 0}, {-107.885, 60}, {-67.8851, -60}, {-27.8851, 60}, {12.1149, -60}, {52.1149, 60}, {72.1149, 0}}, thickness = 5), Line(origin = {-0.02, -1.02}, points = {{-59.9838, -58.9838}, {-19.9838, -48.9838}, {20.0162, -18.9838}, {50.0162, 21.0162}, {60.0162, 61.0162}}, color = {255, 0, 0}, thickness = 3)}),
    Documentation(info = "<html>
<p>
Partial model <code>TabularSpringIcon</code> defines shared icon geometry for tabular spring icon.
</p>
<p>
It contains only graphical annotation primitives. Component and package models extend it to share the BobLib visual language without duplicating icon geometry.
</p>
</html>"));
end TabularSpringIcon;
