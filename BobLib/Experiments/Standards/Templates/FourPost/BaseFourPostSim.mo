...
partial model BaseFourPostSim
  ...
  // FourPostEval record
  import BobLib.Records.StandardRecord.FourPostEvalRecord;
  ...
  replaceable model FrAxleModel =
    BobLib.Chassis.Suspension.FrAxleDW_BC_Stabar
    constrainedby BobLib.Chassis.Suspension.AxleDWBase;
  replaceable model RrAxleModel =
    BobLib.Chassis.Suspension.RrAxleDW_BC_Stabar
    constrainedby BobLib.Chassis.Suspension.AxleDWBase;
  ...
  BobLib.Utilities.Mechanics.MultiBody.Actuators.ChassisActuator frChassisActuator(axleRef = frAxleDW.effectiveCenter) annotation(
    Placement(transformation(origin = {0, 24}, extent = {{10, -10}, {-10, 10}})));

  // Rear chassis actuator
  BobLib.Utilities.Mechanics.MultiBody.Actuators.ChassisActuator rrChassisActuator(axleRef = rrAxleDW.effectiveCenter) annotation(
  ...
  BobLib.Utilities.Mechanics.MultiBody.Actuators.ContactPatchFixture FL_fixture(CP_init = cpInitFL) annotation(
    Placement(transformation(origin = {-40, 20}, extent = {{-20, -20}, {20, 20}})));

  BobLib.Utilities.Mechanics.MultiBody.Actuators.ContactPatchFixture FR_fixture(CP_init = cpInitFR) annotation(
    Placement(transformation(origin = {40, 20}, extent = {{-20, -20}, {20, 20}})));

  BobLib.Utilities.Mechanics.MultiBody.Actuators.ContactPatchFixture RL_fixture(CP_init = cpInitRL) annotation(
    Placement(transformation(origin = {-40, -80}, extent = {{-20, -20}, {20, 20}})));

  BobLib.Utilities.Mechanics.MultiBody.Actuators.ContactPatchFixture RR_fixture(CP_init = cpInitRR) annotation(
    Placement(transformation(origin = {40, -80}, extent = {{-20, -20}, {20, 20}})));

  // Force actuators
  BobLib.Utilities.Mechanics.MultiBody.Actuators.ContactPatchForceActuator FL_ForceActuator(
    fxTable = fxTable,
    fyTable = fyTable,
    forceMagnitude = forceMagnitude / 2) annotation(
    Placement(transformation(origin = {-60, 40}, extent = {{-10, -10}, {10, 10}})));

  BobLib.Utilities.Mechanics.MultiBody.Actuators.ContactPatchForceActuator FR_ForceActuator(
    fxTable = fxTable,
    fyTable = fyTable,
    forceMagnitude = forceMagnitude / 2) annotation(
    Placement(transformation(origin = {60, 40}, extent = {{10, -10}, {-10, 10}})));

  BobLib.Utilities.Mechanics.MultiBody.Actuators.ContactPatchForceActuator RL_ForceActuator(
    fxTable = fxTable,
    fyTable = fyTable,
    forceMagnitude = forceMagnitude / 2) annotation(
    Placement(transformation(origin = {-60, -60}, extent = {{-10, -10}, {10, 10}})));

  BobLib.Utilities.Mechanics.MultiBody.Actuators.ContactPatchForceActuator RR_ForceActuator(
    fxTable = fxTable,
    fyTable = fyTable,
    forceMagnitude = forceMagnitude / 2) annotation(
    Placement(transformation(origin = {60, -60}, extent = {{10, -10}, {-10, 10}})));
