within BobLib.Utilities.Mechanics.MultiBody.ContactMechanics;

model GroundPhysics

  extends BobLib.Icons.GroundPhysicsIcon;
...
<p>
Model <code>GroundPhysics</code> closes a MultiBody contact-patch frame to a
ground reference with a smooth unilateral vertical contact force.
</p>
<p>
The model belongs to the shared MultiBody utility layer because chassis
assemblies own the ground/contact closure. Tire and suspension models expose
raw contact-patch frames and consume the resulting contact forces through those
frames.
</p>
</html>"));
end GroundPhysics;
