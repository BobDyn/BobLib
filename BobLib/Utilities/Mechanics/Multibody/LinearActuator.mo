...
model LinearActuator
  ...
  Modelica.Mechanics.MultiBody.Joints.Prismatic prismatic_x(
    n = {1, 0, 0},
    useAxisFlange = true,
    animation = false) annotation(
    Placement(transformation(origin = {-50, 0}, extent = {{-10, -10}, {10, 10}})));
  Modelica.Mechanics.MultiBody.Joints.Prismatic prismatic_y(
    n = {0, 1, 0},
    useAxisFlange = true,
    animation = false) annotation(
...
equation
  if axis == "x" then
    ...
    connect(position.support, prismatic_x.support);
  elseif axis == "y" then
    connect(position.flange, prismatic_y.axis);
    connect(position.support, prismatic_y.support);
  elseif axis == "z" then
    connect(position.flange, prismatic_z.axis);
    connect(position.support, prismatic_z.support);
  end if;
