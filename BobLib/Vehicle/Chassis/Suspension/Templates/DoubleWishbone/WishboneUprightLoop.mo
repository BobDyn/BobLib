...
model WishboneUprightLoop "Kinematic loop consisting of upright, lower wishbone, and upper wishbone"
  ...
  // Lower wisbone
  Modelica.Mechanics.MultiBody.Joints.Revolute lowerJoint_i(n = Vectors.normalize(pDW.lowerFore_i - pDW.lowerAft_i), cylinderLength = Vectors.norm(pDW.lowerFore_i - pDW.lowerAft_i) + linkDiameter, cylinderDiameter = linkDiameter, phi(nominal = 0.05), w(start = 0, nominal = 1)) annotation(
  ...
  // Steering interface
  Modelica.Mechanics.MultiBody.Joints.Revolute steeringAxis(n = Vectors.normalize(pDW.upper_o - pDW.lower_o), cylinderLength = jointDiameter, cylinderDiameter = jointDiameter, phi(nominal = 0.1), w(start = 0, nominal = 1)) annotation(
  ...
  Modelica.Mechanics.MultiBody.Visualizers.FixedShape upperForeRod(shapeType = "cylinder", lengthDirection = Frames.resolve2(upperFrameToFore.frame_b.R, upperForeLinkDirection), length = upperForeLinkLength, width = linkDiameter, height = linkDiameter, color = {0, 0, 0}) annotation(
    Placement(transformation(origin = {-40, 90}, extent = {{-10, -10}, {10, 10}})));
  Modelica.Mechanics.MultiBody.Visualizers.FixedShape upperAftRod(shapeType = "cylinder", lengthDirection = Frames.resolve2(upperFrameToAft.frame_b.R, upperAftLinkDirection), length = upperAftLinkLength, width = linkDiameter, height = linkDiameter, color = {0, 0, 0}) annotation(
  ...
  Modelica.Mechanics.MultiBody.Visualizers.FixedShape lowerForeRod(shapeType = "cylinder", lengthDirection = Frames.resolve2(lowerFrameToFore.frame_b.R, lowerForeLinkDirection), length = lowerForeLinkLength, width = linkDiameter, height = linkDiameter, color = {0, 0, 0}) annotation(
    Placement(transformation(origin = {-40, -30}, extent = {{-10, -10}, {10, 10}})));
  Modelica.Mechanics.MultiBody.Visualizers.FixedShape lowerAftRod(shapeType = "cylinder", lengthDirection = Frames.resolve2(lowerFrameToAft.frame_b.R, lowerAftLinkDirection), length = lowerAftLinkLength, width = linkDiameter, height = linkDiameter, color = {0, 0, 0}) annotation(
    Placement(transformation(origin = {-40, -90}, extent = {{-10, -10}, {10, 10}})));
protected
